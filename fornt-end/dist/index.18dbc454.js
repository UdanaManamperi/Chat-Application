...
const API_BASE_URL = "ws://localhost:8080/api/v2";
inputElm.addEventListener("change", ()=>{
    if (inputElm.checked) element.dataset.bsTheme = "dark";
    else element.dataset.bsTheme = "light";
});
btnSendElm.addEventListener("click", ()=>{
    ...
    fetch(`${API_BASE_URL}/messages`, {
        ...
        if (res.ok) {
            ...
            txtMessageElm.value = "";
            txtMessageElm.focus();
        } else alert("Failed to send the chat message");
    }).catch((err)=>{
        alert("Failed to connect with the server, please check the connection.");
    });
});
...
},{}]},["iqNlW","1SICI"], "1SICI", "parcelRequire008d")
